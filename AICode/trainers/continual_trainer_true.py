"""
TRUE Continual Learning Trainer
Train ONLY on new data per task, constant time complexity
"""
import torch
...
class TrueContinualTrainer:
    def __init__(self, model, use_replay=True, device="cuda", num_tasks=5, buffer_size=200):
        """
        TRUE Continual Learning trainer with Experience Replay
        ...
        Args:
            ...
            buffer_size: Samples per class in replay buffer (default: 200)
        """
        self.model = model
        ...
        # Experience Replay buffer (CRITICAL for TRUE CL)
        # Increased from 50 to 100 samples per class for better retention
    ...
    def train_all_tasks(self, epochs_per_task=10, batch_size=128, lr=0.01, data_root="./data"):
        """Train on all tasks sequentially - TRUE continual learning style"""
        ...
        for task_id in range(self.num_tasks):
            ...
            # Experience Replay automatically mixes old samples during training
